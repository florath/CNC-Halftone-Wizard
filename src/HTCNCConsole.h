...
/**@brief A singleton that defines the interface to the application error
 * console.
*/
class Console {
public:
  /**@brief Defines the severity levels for messages as well as the display
   * threshold.
   * A message with severity NONE will never be displayed.  A display threshold
   * of
...
*	(Um, yes it is, dude...)
  ...
  /**@brief Sets the minimum severity level a message must have to be displayed
   * in the console.
   */
  void setSeverityThreshold(Severity s) { m_severityOutputThreshold = s; }

  /**@brief Returns the minimum severity level a message must have to be
   ...
   * @sa appendText
   */
  template <typename T> Console &operator<<(const T &);
...
private:
  ...
  /// Controls printing of the timestamp so that it only appears at the start of
  /// a line.
  bool m_printTimestamp;
  bool m_lastTextWasHTML;
};

template <typename T> Console &Console::operator<<(const T &msg) {
  std::stringstream ss;
