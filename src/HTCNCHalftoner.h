...
/*@brief Converts arbitrary images to halftone images as well as CNC
 *instructions.
 **/
class Halftoner {
public:
  ...
  /**
   * @brief Constructs a Halftoner object and performs all the output
   *calculations.
   ...
   **/
  Halftoner(const QPixmap &src, QImage &dest, int scale, bool generateGCode,
            const CNCParameters &params);

  virtual ~Halftoner() {}

  /// Returns the number of cuts (Z up/down movements) needed to make the
  /// image computed in the constructor.
  int getCutCount() const { return m_cutCount; }

  /// Returns the g code needed to do the actual cutter movement (no
  /// pre/post-amble).
  QString getGCode() const { return m_gCode; }
